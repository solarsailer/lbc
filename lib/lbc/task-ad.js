...
export default async (page, data, {phone}) => {
  console.log('Automating form…')
  ...
  for (let i = 0; i < data.images.length; i++) {
    const element = await page.$(`#image${i}`)

    const image = data.images[i]
    console.info(chalk.dim(`Uploading '${image}'…`))

    await element.uploadFile(image)
    await page.waitForSelector(`#uploadPhoto-${i}[data-state="uploaded"]`)
  ...
  // Phone.
  await page.type($USER_PHONE, phone)
